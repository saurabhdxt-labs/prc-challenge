...
Deliberately NOT here: seed averaging, per-airport models, the stand block. None of them is
measured on LightGBM; they are v4. A smoke run proves the code path and nothing else - it
prints a banner saying so, because this project has twice quoted a smoke magnitude as a
result.
...
#: the smoke deviates only where a code-path check needs it to finish in seconds
SMOKE = dict(months=(1, 3), n_rank=5_000, learning_rate=0.05, nest=60, patience=50)
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    ap.add_argument("--reuse-booster", action="store_true",
                    help="skip the fit if the saved booster and its fit.json exist")
...
def recover_taxi_time(proxy, delta_hat) -> np.ndarray:
    ...
    if it binds on more than 0.5% of rows the delta model is broken, not merely imprecise.
    """
...
def load_frames(cache_dir: pathlib.Path, months=None, n_rank=None, seed=0):
...
def fit_and_refit(X, dlt, y, proxy, train, fit, es, params, nest, patience):
    ...
    log(f"early-stopping run: fit {fit.sum():,} rows, stop on {es.sum():,} rows "
        f"(months {ES_MONTHS}), lr {params['learning_rate']}, max {nest:,} rounds, "
        f"patience {patience}")
    ...
    ds = lgb.Dataset(X[train], dlt[train])
    b = lgb.train(params, ds, num_boost_round=n_ref, callbacks=[lgb.log_evaluation(0)])
    del ds
    gc.collect()
    log(f"refit done: {b.num_trees():,} trees   peak RSS {peak_rss_gb():.2f} GB")
    info = dict(best_iter=best_iter, n_ref=n_ref, n_fit=int(fit.sum()), n_es=int(es.sum()),
                n_all=int(train.sum()), es_months=list(ES_MONTHS),
                es_rmse_taxi_time_NOT_A_RESULT=es_rmse, es_proxy_only_rmse=proxy_only)
    return b, info


def check_provenance(info: dict, n_trees: int, smoke: bool, n_all: int, params: dict) -> None:
    ...
    parameters, or a truncated model file would otherwise be spliced into a submission
    silently. Refuse on any mismatch.
    """
    problems = []
...
def main(argv=None) -> int:
    ...
    booster_path = (out_dir if smoke else CACHE) / f"lgbm_v{args.version}.txt"
    fit_json = booster_path.with_suffix(".fit.json")
    log(f"version {args.version}  smoke={smoke}  out={dest}  booster={booster_path}")

    # ---- 1. rows ----
    d, is_rank, rank_ids = load_frames(CACHE, months=months, n_rank=n_rank)
    ...
    # ---- 4. fit, or reuse a saved booster ----
    if args.reuse_booster and booster_path.exists() and fit_json.exists():
        booster = lgb.Booster(model_file=str(booster_path))
        info = json.loads(fit_json.read_text())
        check_provenance(info, booster.num_trees(), smoke, int(train.sum()), params)
        log(f"reused booster {booster_path} ({booster.num_trees():,} trees), best_iter "
            f"{info['best_iter']:,}, n_ref {info['n_ref']:,}")
    else:
        t_fit = time.time()
        booster, info = fit_and_refit(X, dlt, y, proxy, train, fit, es, params, nest, patience)
        info.update(fit_wall_s=round(time.time() - t_fit, 1), smoke=smoke, params=params)
        booster.save_model(str(booster_path))
        fit_json.write_text(json.dumps(info, indent=2))
        log(f"saved booster -> {booster_path}  fit wall {info['fit_wall_s']:.0f}s")
    ...
    # ---- 6. splice into the base submission ----
    ...
    meta = {"version": args.version, "file": dest.name, "base": base_path.name, "smoke": smoke,
            **_git(), "started_utc": started.isoformat(), "finished_utc": finished.isoformat(),
            ...
            "peak_rss_gb": round(peak_rss_gb(), 2), **info,
